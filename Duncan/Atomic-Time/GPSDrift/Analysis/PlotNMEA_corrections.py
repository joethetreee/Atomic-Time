...
linesGGA=[]
timeS=[]								# Sum time (length of GPS second)
timeR=[]								# Reference time for each measurement
...
for i in range(len(lines)):
	if (len(lines[i])>(6+14)):			# check whether line is long enough
		if (lines[i][:6]=="$GPGGA"):	# check that the format is GPGGA
			...
			if (i==0):
				timeS.append(int(contents[commaLoc+1:]))
			else:
				timeS.append(int(contents[commaLoc+1:])+timeS[-1])
...
timeR=[i+1 for i in range(len(linesGGA))]		# reference time for each measurement
qCon=[0 for i in range(len(linesGGA))]			# quality of connection
			
timeAvg = timeS[-1]/timeR[-1]
print (timeAvg)
	
print ("timeR[-1]: ",timeR[-1])
# correct for skipped seconds	
changed = True
while (changed==True):							# iterate procedure until finished
	timeOffset = 0								# number of seconds skipped/double-counted (+/-)
	changed = False
	for i in range(len(timeR)):
		timeDifference = 0
		if (i==0):
			timeDifference = timeS[i]
		else:
			timeDifference = timeS[i]-timeS[i-1]
		timeOffset = round((timeDifference-timeAvg)/1000)

		if (i>4 and i<len(timeR)-5):			# check whether the time went back immediately
			if (timeOffset!=0):
				if (round((timeS[i+5]-timeS[i-5]-2*timeAvg)/1000)==10):
					timeOffset=0
		
		timeTemp = timeR[i]
		timeR[i] = 1+i+timeOffset
		
		if (timeTemp!=timeR[i]):
			changed = True
			#print("changed: ", timeRTemp, " to ", timeR[i])
		if (timeOffset!=0):
			print("td ", timeS[i+5], timeS[i],timeS[i-5])
	timeAvg = timeS[-1]/timeR[-1]
	print ("timeR[-1]: ",timeR[-1])
	print(timeAvg)
	print(timeOffset)
...
for i in range(len(linesGGA)):					# get information for timeV and qCon
	...
	timeV[i]=timeS[i]-timeR[i]*timeAvg
	
		
plt.plot(timeR, timeV)
plt.title("Variation in arrival time of GPS data")
