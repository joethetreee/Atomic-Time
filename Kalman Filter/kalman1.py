...
xhat = numpy.matrix([serial[0]])
P		= numpy.matrix([arduinoUncertainty])
...
plt.plot(range(len(kalman)), kalman, color = "red", label = "Kalman Filtered")
plt.scatter(range(len(serial)), serial, label = "Raw")
plt.show()